...
function sigmoidCNN(layer,input_size, n_integer, n_decimal)
    name = sprintf('CNN_Network/CNN/sigmoidL%d.vhd', layer);
    fid = fopen(name, 'wt');
    fprintf(fid, '--------------------------Sigmoid------------------------------------\n');
    ...
    fprintf(fid, '\n');
    fprintf(fid, 'entity sigmoid_L%d is\n', layer);
    fprintf(fid, '\tPort ( \n'); 
    ...
    fprintf(fid, 'end sigmoid_L%d;\n', layer);
    fprintf(fid, '\n');
    fprintf(fid, 'architecture Behavioral of sigmoid_L%d is\n', layer);
    fprintf(fid, '\n');
    fprintf(fid, 'Begin\n');
    ...
    for i = 1: (2^input_size )
        fprintf(fid, '"%s" when "%s",\n', dec2q(sigmoid(origin(i , 1)), n_integer,n_decimal, 'bin'), dec2q(origin(i, 1), n_integer,n_decimal, 'bin'));
    end
